...
class Sprite:	
	def __init__(self, parent, frames, animations):
		...
		#self.map = True	#needed when map bigger than one screen/camera movement
	...
	def Render(self):
		...
		#if self.map:	#needed when map bigger than one screen/camera movement
	...
	def LabelChanged(self, newLabel):
		...
		if not newLabel in self.animations:
			#need to check normal state first for changing directions mid-air
			if 'Left' in self.parent.GetState():
				newLabel += 'Left'
			elif 'Right' in self.parent.GetState():
				newLabel += 'Right'
			elif 'Left' in self.currentLabel:
				newLabel += 'Left'
			else:
				newLabel += 'Right'
		if not newLabel in self.animations: #still.....
